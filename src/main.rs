...
impl olc::PGEApplication for Emulator {
    ...
    fn on_user_create(&mut self, _pge: &mut olc::PixelGameEngine) -> bool {
        self.ram.load_from_rom(0x000,PathBuf::from("system/font.bin"));
    ...
    fn on_user_update(&mut self, pge: &mut olc::PixelGameEngine, delta: f32) -> bool {
        ...
        if self.run_mode == RUN::CONT { // run continuously at 600 CPS
            ...
            if self.cycle_time >= SECONDS_PER_CYCLE {
                ...
                if redraw { self.draw(pge); }
                self.draw_debug(pge,summary);
                self.cycle_time = 0.0;
            }
            if pge.get_key(olc::Key::Space).pressed {self.run_mode = RUN::STEP}
            ...
            if pge.get_key(olc::Key::Space).pressed {self.run_mode = RUN::CONT}
        }
        true
    }
}
...
fn main() {
    olc::PixelGameEngine::construct(Emulator::new(),(SCR_W + (SCR_W / 2))*DRAW_BIGGER_PIXELS as usize,(SCR_H + (SCR_H / 2))*DRAW_BIGGER_PIXELS as usize,2,2).start();
}
...
impl Emulator {
    ...
    fn draw (&mut self, pge: &mut olc::PixelGameEngine) {
        let bigger_draw = if self.run_mode == RUN::STEP {DRAW_BIGGER_PIXELS} else {DRAW_BIGGER_PIXELS + (DRAW_BIGGER_PIXELS / 2)};
            ...
            for y in 0..32 {
                for xs in 0..bigger_draw {
                    for ys in 0..bigger_draw {
                        ...
                        pge.draw(x as i32 * bigger_draw + xs, y as i32 * bigger_draw + ys, pixel);
                    }
                }
            }
        }
    ...
    fn draw_debug (&mut self, pge: &mut olc::PixelGameEngine, summary: String) {
        if self.run_mode == RUN::STEP {
            for i in 0..0x8 {
                ...
                pge.draw_string(64 * DRAW_BIGGER_PIXELS + 4, 4 + (i * 8), &string, olc::WHITE);
            }
            for i in 0..0x8 {
                let mut string = String::new();
                string += format!("R{:1X}:", 0x8 + i).as_str();
                string += format!("{:2X}", self.registers.get(0x8 + i as u8)).as_str();
                pge.draw_string(64 * DRAW_BIGGER_PIXELS + 64, 4 + (i * 8), &string, olc::GREY);
            }
            let (stringa, stringb) = summary.split_once(" => ").unwrap();
            pge.draw_string(4, 32 * DRAW_BIGGER_PIXELS + 8, &stringa.to_string(), olc::WHITE);
            pge.draw_string(4, 32 * DRAW_BIGGER_PIXELS + 16, &stringb.to_string(), olc::WHITE);
        }
    }
}