...
impl Emulator { // implement all of the instruction code here, to keep main less cluttered
    ...
    match instruction.opcode {
        ...
        0x7 => { // IMM ADD RX + byte => RX
            let (result, overflow) = x.overflowing_add(byte);
            self.registers.set(x_reg,result);
            summary += format!("IMM ADD R{:1X} + {:#4X} => R{:1X}",x_reg,byte,x_reg).as_str();
        },
        0x8 => { // ALU stuff
            match n {
                0x0 => { // COPY RY => RX
                ...
                0x1 => { // OR RX | RY => RX
                    ...
                    summary += format!("OR R{:1X} | R{:1X} => R{:1X}",x_reg,y_reg,x_reg).as_str();
                },
                0x2 => { // AND RX & RY => RX
                    self.registers.set(x_reg, x & y);
                    summary += format!("AND R{:1X} & R{:1X} => R{:1X}",x_reg,y_reg,x_reg).as_str();
                },
                0x3 => { // XOR RX ^ RY => RX
                    self.registers.set(x_reg, x ^ y);
                    summary += format!("XOR R{:1X} ^ R{:1X} => R{:1X}",x_reg,y_reg,x_reg).as_str();
                },
                0x4 => { // ADD RX + RY => RX (sets overflow flag)
                    let (result, overflow) = x.overflowing_add(y);
                    self.registers.set(0xF, if overflow {1} else {0});
                    self.registers.set(x_reg,result);
                    summary += format!("ADD R{:1X} + R{:1X} => R{:1X}",x_reg,y_reg,x_reg).as_str();
                },
                0x5 => { // SUB RX - RY => RX (sets !overflow flag)
                    let (result, overflow) = x.overflowing_sub(y);
                    self.registers.set(0xF, if overflow {0} else {1});
                    self.registers.set(x_reg,result);
                    summary += format!("SUB R{:1X} - R{:1X} => R{:1X}",x_reg,y_reg,x_reg).as_str();
                },
                0x6 => { // SHR RX >> 1 => RX (sets overflow flag)
                    self.registers.set(0xF, x & 1);
                    self.registers.set(x_reg, x >> 1);
                    summary += format!("SHR R{:1X} >> 1 => R{:1X}",x_reg,x_reg).as_str();
                },
                0x7 => { // SUB RY - RX => RX (sets !overflow flag)
                    let (result, overflow) = y.overflowing_sub(x);
                    self.registers.set(0xF, if overflow {0} else {1});
                    self.registers.set(x_reg,result);
                    summary += format!("SUB R{:1X} - R{:1X} => R{:1X}",y_reg,x_reg,x_reg).as_str();
                },
                0xE => { // SHL RX << 1 => RX (sets overflow flag)
            ...
            match byte {
                ...
                0x55 => { // STORE R0..RX
                    for i in 0..=x_reg as usize {
                        self.ram.set(self.stack_pointer + i as u16, self.registers.get(i as u8));
                    }
                    summary += format!("STORE R0..R{:1X}",x_reg).as_str();
                },
                0x65 => { // LOAD R0..RX
                    for i in 0..=x_reg as usize {
                        self.registers.set(i as u8, self.ram.get(self.stack_pointer + i as u16));
                    }
                    summary += format!("STORE R0..R{:1X}",x_reg).as_str();
                },
                _ => { summary += "???"; }
